...
# ============================================================
# ROBUST GOOGLE DRIVE ONNX MODEL DOWNLOADER
# ============================================================
...
def _gdrive_download(file_id: str, dest_path: str) -> bool:
    """
    Robust Google Drive large-file downloader.
    Handles the virus-scan confirmation page that breaks simple uc?export=download URLs.
    ...
    # Step 2 — detect the virus-scan confirmation page and extract the token
    confirm_token = None

    # Method A: cookie-based token (older GDrive behaviour)
    for key, value in resp.cookies.items():
        if "download_warning" in key:
            confirm_token = value
            break

    # Method B: form-field token embedded in the HTML (newer GDrive behaviour)
    if not confirm_token:
        content_type = resp.headers.get("Content-Type", "")
        if "text/html" in content_type:
            html_text = resp.text
            # Look for confirm=<token> in the page source
            import re
            match = re.search(r'confirm=([0-9A-Za-z_\-]+)', html_text)
            if match:
                confirm_token = match.group(1)
            # Also look for the newer &uuid= style redirect URL
            uuid_match = re.search(r'https://drive\.google\.com/uc\?[^"\']*confirm=([^"\'&]+)', html_text)
            if uuid_match and not confirm_token:
                confirm_token = uuid_match.group(1)

    # Step 3 — if a confirmation token was found, re-request with it
    if confirm_token:
        params = {"id": file_id, "confirm": confirm_token}
        resp = session.get(URL, params=params, stream=True, timeout=120)
        resp.raise_for_status()

    # Step 4 — validate we're getting binary (not an error HTML page)
    content_type = resp.headers.get("Content-Type", "")
    if "text/html" in content_type:
        # Google may now use a different export endpoint for large files
        # Try the newer direct-download endpoint
        alt_url = f"https://drive.google.com/uc?id={file_id}&export=download&confirm=t"
        resp = session.get(alt_url, stream=True, timeout=120)
        resp.raise_for_status()
        content_type = resp.headers.get("Content-Type", "")
        if "text/html" in content_type:
            raise ValueError(
                "Google Drive returned an HTML page instead of the binary file. "
                "The file may be private, deleted, or too large for direct export. "
                "Please make sure the file is publicly accessible ('Anyone with the link')."
            )

    # Step 5 — stream binary data to disk
    ...
    if total_bytes < 1024 * 10:  # sanity: reject files under 10 KB
        os.remove(tmp_path)
        raise ValueError(
    ...
    # Validate it starts with a valid ONNX/protobuf magic byte sequence
    with open(tmp_path, "rb") as f:
        header = f.read(8)
    # ONNX protobuf files start with 0x08 (field 1, varint wire type)
    # They are NOT ZIP or PNG or HTML — reject obvious wrong formats
    if header[:4] in (b'%PDF', b'\x89PNG', b'PK\x03\x04') or header[:5] == b'<html':
        os.remove(tmp_path)
        raise ValueError(
            f"Downloaded content does not appear to be a valid ONNX model "
            f"(got header: {header!r}). Google Drive may have served a preview/error page."
        )
...
if not os.path.exists(MODEL_PATH):
    with st.spinner("📥 Synchronizing optimized ONNX model weights from cloud storage… please wait."):
        ...
        try:
            _gdrive_download(FILE_ID, MODEL_PATH)
            st.success("✅ Model weights synchronized successfully!")
            st.rerun()
        except Exception as e:
            st.error(
                f"❌ Cloud model download failed: {e}\n\n"
                "**Troubleshooting steps:**\n"
                "1. Make sure the Google Drive file is shared as **'Anyone with the link can view'**.\n"
                "2. If the file is >100 MB, Google Drive may block direct downloads — "
                "consider hosting on Hugging Face Hub or a direct S3/R2 bucket instead.\n"
                "3. You can also manually place `xray_model_best.onnx` at "
                f"`{MODEL_PATH}` on the server."
            )
            st.stop()
...
.xray-panel-wrap{background:#0a1628;border-radius:14px;border:1px solid rgba(45,212,191,.20);overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.6),inset 0 1px 0 rgba(255,255,255,.04);}
.xray-panel-header{padding:10px 16px;background:rgba(13,148,136,.08);border-bottom:1px solid rgba(45,212,191,.15);font-family:monospace;font-size:11px;font-weight:bold;letter-spacing:2px;text-transform:uppercase;color:#94a3b8;display:flex;align-items:center;gap:8px;}
.xray-panel-header.teal{color:#2dd4bf;}
.xray-panel-body{position:relative;width:100%;background:#050e1a;}
.xray-panel-body img{width:100%;height:260px;object-fit:cover;display:block;}
.consolidation-badge{position:absolute;bottom:12px;right:12px;background:linear-gradient(135deg,#dc2626,#991b1b);color:#fff;font-family:monospace;font-size:10px;font-weight:bold;letter-spacing:1.5px;padding:5px 12px;border-radius:6px;border:1px solid rgba(252,165,165,.4);box-shadow:0 4px 16px rgba(220,38,38,.5),0 0 24px rgba(220,38,38,.3);text-transform:uppercase;animation:fadeUp .5s ease both;}
.normal-badge{position:absolute;bottom:12px;right:12px;background:linear-gradient(135deg,#065f46,#064e3b);color:#6ee7b7;font-family:monospace;font-size:10px;font-weight:bold;letter-spacing:1.5px;padding:5px 12px;border-radius:6px;border:1px solid rgba(110,231,183,.35);box-shadow:0 4px 16px rgba(6,95,70,.5);text-transform:uppercase;animation:fadeUp .5s ease both;}
.focus-label{position:absolute;top:10px;left:10px;background:rgba(0,0,0,.65);color:#2dd4bf;font-family:monospace;font-size:9px;letter-spacing:1px;padding:3px 8px;border-radius:4px;border:1px solid rgba(45,212,191,.3);}

.cbubble{background:linear-gradient(165deg,rgba(45,212,191,.08) 0%,rgba(13,148,136,.03) 100%);backdrop-filter:blur(16px);border:1px solid rgba(45,212,191,.16);border-left:4px solid #2dd4bf;padding:18px;border-radius:12px;font-family:sans-serif;font-size:13px;line-height:1.7;color:#e2f8f0;margin-bottom:12px;animation:fadeUp .7s ease both;box-shadow:0 8px 24px rgba(0,0,0,.3);}
.disc{background:linear-gradient(135deg,rgba(30,27,11,.9),rgba(40,33,10,.7));border:1px solid #78350f;color:#fcd34d;padding:10px 16px;border-radius:10px;font-family:monospace;font-size:11px;letter-spacing:.5px;margin-bottom:18px;}
.shead{background:linear-gradient(165deg,rgba(45,212,191,.08) 0%,rgba(5,150,105,.03) 100%);backdrop-filter:blur(18px);border:1px solid rgba(45,212,191,.18);padding:22px 24px;border-radius:16px;margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;animation:fadeUp .6s ease both;box-shadow:0 14px 40px rgba(0,0,0,.4),inset 0 1px 0 rgba(255,255,255,.05);position:relative;overflow:hidden;}
.shead::before{content:"";position:absolute;top:-60%;left:-10%;width:50%;height:220%;background:linear-gradient(120deg,rgba(45,212,191,0) 0%,rgba(45,212,191,.07) 50%,rgba(45,212,191,0) 100%);transform:rotate(15deg);pointer-events:none;}
.scap{font-size:11px;font-family:monospace;color:#5eead4;font-weight:bold;text-transform:uppercase;letter-spacing:1.5px;display:inline-block;padding-bottom:4px;border-bottom:2px solid rgba(45,212,191,.25);margin-bottom:4px;}
...
div[data-testid="stAlertContainer"]{background:linear-gradient(165deg,rgba(45,212,191,.08) 0%,rgba(13,148,136,.03) 100%) !important;border:1px solid rgba(45,212,191,.18) !important;border-left:4px solid #2dd4bf !important;border-radius:12px !important;color:#e2f8f0 !important;box-shadow:0 8px 24px rgba(0,0,0,.3) !important;}
...
def load_model_cached():
    ...
    for p in paths:
        if os.path.exists(p):
            # Validate before loading — catch corrupt files early with a clear message
            with open(p, "rb") as f:
                header = f.read(8)
            if header[:5] in (b'<html', b'<!DOC') or header[:4] == b'%PDF':
                st.error(
                    f"❌ Model file at `{p}` appears to be an HTML/error page, not a valid ONNX model.\n\n"
                    "This usually means the Google Drive download was intercepted by a virus-scan page.\n\n"
                    "**Fix:** Delete the corrupt file and restart the app — the improved downloader will re-fetch it."
                )
                # Delete the bad file so the downloader retries on next restart
                try:
                    os.remove(p)
                except Exception:
                    pass
...
# ============================================================
# GRAD-CAM (kept for compatibility; actual GradCAM comes from XRayClassifier)
# ============================================================
def red_hot_lut():
    ...
    for i in range(256):
        ...
        if t < 0.5:
            ...
            r = 255; g = int(165 * ((t - 0.5) / 0.35)); b = 0
        else:
            r = 255; g = int(165 + (255 - 165) * ((t - 0.85) / 0.15)); b = int(255 * ((t - 0.85) / 0.15))
        lut[i, 0] = [b, g, r]
    return lut
...
def focus_region_label(hm):
    ...
    total = hm.sum()
    if total <= 0:
        return "Diffuse / No Dominant Region"
    ys, xs = np.mgrid[0:h, 0:w]
    ...
    if hz == "Central" and v == "Mid":
        return "Central / Diffuse"
    return f"{hz} {v} Field"
...
def focus_box(hm):
    ...
    total = float(hm.sum())
    if total <= 1e-6:
        return (0, 0, 1, 1), (0.5, 0.5), False
    ys, xs = np.mgrid[0:h, 0:w]
    cy = float((ys * hm).sum() / total) / h
    cx = float((xs * hm).sum() / total) / w
    vy = float((((ys / h) - cy) ** 2 * hm).sum() / total)
    vx = float((((xs / w) - cx) ** 2 * hm).sum() / total)
    sy = max(np.sqrt(vy), 0.06); sx = max(np.sqrt(vx), 0.06)
    x0 = max(0, cx - 1.5 * sx); x1 = min(1, cx + 1.5 * sx)
    y0 = max(0, cy - 1.5 * sy); y1 = min(1, cy + 1.5 * sy)
...
def type_chip(t):
    return f'<span class="type-chip {_TYPE_CHIP.get(t, "chip-unkn")}">&#9877; {t}</span>'
...
def make_pdf(patient, prob, focus, ptype, side, raw_img, overlay_img, loc_fig, is_pos):
    ...
    patient_tbl = Table([
        ...
        [Paragraph(patient["name"], BD), Paragraph(f"{patient['age']} yrs / {patient['gender']}", BD), Paragraph(patient.get("location", "N/A"), BD)],
        [Paragraph("EXAMINATION DATE", ML), Paragraph("PNEUMONIA TYPE (USER-ENTERED)", ML), Paragraph("AFFECTED SIDE (USER-ENTERED)", ML)],
        [Paragraph(datetime.now().strftime("%d %B %Y"), BD), Paragraph(ptype, BD), Paragraph(side, BD)],
    ], colWidths=[W / 3.0] * 3)
    ...
    result_tbl = Table([
        ...
        [Paragraph(result_label, RESULT), Paragraph(f"{confidence * 100:.1f}%", CONF), Paragraph(focus_text, TL if is_pos else BODY)],
    ...
    if is_pos:
        impression_txt = (
            ...
            f"clinical finding, and the pneumonia type/side shown above were entered by the user, not derived by the model."
        )
    else:
        impression_txt = (
    ...
    disc_txt = (
        ...
        "by a licensed physician.\n\nVisual overlays (such as Grad-CAM) represent computational regions of focus; "
        "they do not indicate confirmed pathology. Clinical parameters including suspected pneumonia type and "
        "anatomical location are user-entered and not independently extracted from the image. "
        "All findings require mandatory correlation with clinical symptoms and verification by a licensed healthcare provider."
    )
    ...
    Research Demo &mdash; Complete all fields before running pipeline</div>
            ...
            if go:
                if not uploaded:
                    ...
                    st.session_state.p_side = p_side
...
<div class="chart-grid">
  <div class="chart-card"><div class="chart-title">Bar Chart</div><canvas id="barChart" width="240" height="200"></canvas></div>
  <div class="chart-card"><div class="chart-title">Gauge Chart</div><canvas id="gaugeChart" width="240" height="200"></canvas></div>
  <div class="chart-card"><div class="chart-title">Pie Chart</div><canvas id="pieChart" width="240" height="200"></canvas></div>
</div>
<script>
(function(){{
...
(function(){{
  ...
  ctx.clearRect(0,0,W,H);
  var slices=[{{val:pneu,col:pcol}},{{val:norm,col:"rgba(30,58,47,.9)"}}];
  var start=-Math.PI/2;
  ...
  [{{col:pcol,lbl:"Pneumonia"}},{{col:"rgba(30,58,47,.9)",lbl:"Normal"}}].forEach(function(l,i){{
    ...
    with st.container(border=True):
        ...
        st.markdown(
            ...
            '<span style="font-family:monospace;font-size:12px;font-weight:bold;color:#fca5a5;letter-spacing:1.5px;text-transform:uppercase;">Important Medical Disclaimer</span>'
            '</div>'
            '<p style="margin:0 0 12px;color:#fcd34d;font-weight:600;">&#9888; This tool does NOT provide medicine recommendations, prescriptions, or treatment advice.</p>'
            '<p style="margin:0 0 12px;">Pneumonia treatment varies significantly based on the type (bacterial, viral, fungal), severity, patient age, and underlying health conditions. Only a licensed medical professional can evaluate your condition and prescribe appropriate treatment.</p>'
            '<p style="margin:0 0 16px;">Please consult a qualified physician or visit a hospital immediately if you suspect pneumonia.</p>'
            '<div style="display:flex;gap:12px;flex-wrap:wrap;">'
            '<a href="https://www.who.int/news-room/fact-sheets/detail/pneumonia" target="_blank" style="display:inline-flex;align-items:center;gap:6px;background:rgba(45,212,191,.12);color:#2dd4bf;border:1px solid rgba(45,212,191,.3);border-radius:8px;padding:8px 16px;font-family:monospace;font-size:11px;font-weight:bold;text-decoration:none;letter-spacing:.5px;">WHO &mdash; Pneumonia Info</a>'
            '<a href="https://www.cdc.gov/pneumonia" target="_blank" style="display:inline-flex;align-items:center;gap:6px;background:rgba(45,212,191,.12);color:#2dd4bf;border:1px solid rgba(45,212,191,.3);border-radius:8px;padding:8px 16px;font-family:monospace;font-size:11px;font-weight:bold;text-decoration:none;letter-spacing:.5px;">CDC &mdash; Pneumonia Guide</a>'
            '<a href="https://www.nhp.gov.in" target="_blank" style="display:inline-flex;align-items:center;gap:6px;background:rgba(45,212,191,.12);color:#2dd4bf;border:1px solid rgba(45,212,191,.3);border-radius:8px;padding:8px 16px;font-family:monospace;font-size:11px;font-weight:bold;text-decoration:none;letter-spacing:.5px;">NHP India &mdash; Health Portal</a>'
            '</div>'
            '</div>',
            unsafe_allow_html=True
        )
...
and NOT prescribing — gather information and help the patient understand how urgently
they should seek in-person medical care.

TRIAGE CHECKLIST (ask 1-2 questions per turn naturally):
- Onset & duration, fever, cough character/color, breathing difficulty, chest symptoms,
  energy/appetite, risk factors, oxygen saturation if available.

RED-FLAG ESCALATION — recommend urgent/emergency care if:
  severe breathlessness, blue/grey lips, confusion, severe chest pain, high fever with rigors,
  coughing blood, dehydration signs, young child/elderly/pregnant with worsening symptoms.

RULES:
1. Warm, empathetic, clear like a real doctor.
2. Ask 1-2 focused follow-up questions per turn.
3. NEVER prescribe specific medications or dosages.
4. Always recommend consulting a real physician for diagnosis and treatment.
5. If ANY red-flag symptom: prioritize urgent care guidance.
6. Keep responses concise, 3-6 sentences max.
